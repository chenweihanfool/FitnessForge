...
export const changelog: ChangelogEntry[] = [
  {
    version: "v3.31",
    date: "2026-09-12",
    title: "修掉活動量公式走越多步分數反而變低的 bug；有氧分數整體放大 4 倍校正跟力量的量級落差",
    items: [
      "修 bug：活動量（步數）公式 (每日步數/500)×(1-0.00002×每日步數) 是一條開口向下的拋物線，峰值在 25000 步，超過之後分數開始倒退、5 萬步歸零、再往上變負數——實測算出 6 萬步/天週基準值 -403.2、8 萬步/天 -1612.8，會拖累當週跟力量分/有氧分加權相加的總分。改成把公式吃到的步數本身封頂在曲線峰值 25000，超過一律當 25000 算，確保分數只會越走越高或持平，不會再倒退或變負",
      "有氧公式（跑步/跑步機負重/開合跳/其他有氧）的固定乘數統一放大 4 倍（1.83→7.3、2.2→8.8）：健檢發現同樣一週練 3 次、時間強度都算紮實的訓練量，力量分數量級是有氧的 4 倍左右，且力量的徒手動作參數改採體重公斤當量校準後（v3.25 前後）這個落差更明顯——放大後同樣的一週 3 次有氧訓練量級跟力量打平。intensityFactor（強度因子）維持原本角色，繼續負責同樣是有氧但強度不同的區分，不受這次調整影響",
      "50%/30%/20%（力量/有氧/活動量）這組混合成單一週總分的權重維持不動——那是「設定」頁面本來就能自己調的個人偏好，跟這次校正的是「三個類別原始分數量級有沒有對齊」不同層次的東西，沒有一併改",
    ],
  },
  {
    version: "v3.30",
