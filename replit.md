...
- **Category-Specific Scoring Formulas**: Each exercise category uses a distinct scoring formula stored as `baselineValue` at entry creation:
  ...
  - **Cardio (有氧)**: General formula: `value(minutes) × sets × intensityFactor × 2.2`. Special formulas: 開合跳 uses `(total_reps × 2 × intensityFactor × 2.2) / 10`. **跑步/跑步機負重 dual-input formula**: `(minutes + km × 10) × intensityFactor × 1.83` — accepts both duration (value=minutes) and distance (sets=km). When km not provided, estimates from default pace (跑步: 12 min/km, 跑步機負重: 20 min/km). UI shows "距離（公里）" label for running exercises instead of "組數". Calibrated for strength:cardio best-week ratio ≈ 3:2.
...
- **AI Weekly Training Plan**: AI-generated personalized weekly training schedules using OpenAI (via Replit AI Integrations). Uses **rolling 8-week average** (one mesocycle) instead of all-time career average as the baseline. Users choose between "恢復周" (recovery: targets = rolling avg × 75%) or "正常周" (normal: targets = rolling avg × 105%, progressive overload). The system generates a day-by-day schedule distributing exercises across 3-5 training days. Dashboard card shows live completion tracking with progress bars per exercise. Plan data stored in `weekly_plans` table (weekStart PK, mode, planJson, generatedAt). API: `GET /api/plan/current`, `POST /api/plan/generate`, `GET /api/plan/progress`, `GET /api/plan/recommend-mode`. Completion threshold is 80% of target to count as "met". The recommend-mode endpoint uses rolling 8-week total baseline to evaluate recent trends vs rolling average (not career average).
...
- **OpenAI**: AI integrations for weekly training plan generation (via Replit AI Integrations, no API key needed).
