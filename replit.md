...
- **AI Weekly Training Plan**: AI-generated personalized weekly training schedules using OpenAI (via Replit AI Integrations). Users choose between "恢復周" (recovery: targets = career averages) or "正常周" (normal: targets = averages + 15%). The system generates a day-by-day schedule distributing exercises across 3-5 training days. Dashboard card shows live completion tracking with progress bars per exercise. Plan data stored in `weekly_plans` table (weekStart PK, mode, planJson, generatedAt). API: `GET /api/plan/current`, `POST /api/plan/generate`, `GET /api/plan/progress`. Completion threshold is 80% of target to count as "met".
